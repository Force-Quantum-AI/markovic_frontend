...
const filterUsers = (
  ...
  if (year) {
    filtered = filtered.filter((user) => user.createdDate.includes(year));
  }
...
export default function UsersTable({ name, year, email }: UsersTableProps) {
  ...
  const [prevFilters, setPrevFilters] = useState({ name, year, email });
  if (
    prevFilters.name !== name ||
    prevFilters.year !== year ||
    prevFilters.email !== email
  ) {
    setPrevFilters({ name, year, email });
    setCurrentPage(1);
  }
    ...
    <div className="w-full mx-auto p-4 md:p-6 space-y-4 bg-white rounded-2xl">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
            {currentUsers.map((user) => (
              <tr
                onClick={() => router.push(`/my-cases/${user.id}`)}
                key={user.id}
              ...
              >
                ...
                <td className="py-4 px-5 text-right">
                  <DropdownMenu>
                    ...
                    >
                      <DropdownMenuItem
                        onClick={() => handleView(user.id, user.name)}
                        className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors focus:bg-gray-50 focus:outline-none"
                      >
                      ...
                      <DropdownMenuItem
                        ...
                        className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 rounded-lg hover:bg-red-50 cursor-pointer transition-colors focus:bg-red-50 focus:outline-none"
                      >
      ...
      <div className="block lg:hidden space-y-4">
        {currentUsers.map((user) => (
          <div
            key={user.id}
            className="w-full bg-white border border-gray-200 rounded-2xl p-4 shadow-sm relative space-y-3"
          >
            ...
            <div className="flex items-start justify-between">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  className="w-32 bg-white rounded-xl shadow-md border p-1"
                >
                  <DropdownMenuItem
                    onClick={() => handleView(user.id, user.name)}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                className={`flex h-9 w-fit px-2 items-center gap-2 justify-center rounded-full transition-all ${
                  currentPage === 1
