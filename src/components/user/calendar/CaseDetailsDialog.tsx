...
export default function CaseDetailsDialog({
  ...
  const { data: caseData, isLoading, isError } = useGetCasesDetailsQuery(
    { caseId: caseId || "" },
    { skip: !caseId }
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-xl md:max-w-2xl border-gray-100 max-h-[90vh] overflow-y-auto p-6 rounded-2xl">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              ...
              <DialogTitle className="text-xl md:text-2xl font-bold text-gray-900 mt-1">
                {isLoading ? "Loading Case Details..." : caseData?.case_name || "Case Details"}
              </DialogTitle>
            </div>
            {!isLoading && caseData && (
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm font-medium text-gray-500">Fetching case information...</p>
          </div>
        ) : isError ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <ShieldAlert className="w-12 h-12 text-red-500 mb-3" />
            <p className="text-sm font-semibold text-gray-800">Failed to load case details</p>
          ...
          <div className="space-y-6 pt-4">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
              <Avatar className="w-16 h-16 bg-gray-200 border-2 border-white shadow-sm shrink-0">
                <AvatarImage src={getClientImageUrl(caseData.client_image)} alt={caseData.client_name} />
              ...
              <div className="flex-1 text-center sm:text-left space-y-2 min-w-0">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Personal ID: <span className="font-semibold text-gray-700">{caseData.personal_id}</span>
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1.5 text-xs text-gray-600">
                  <div className="flex items-center justify-center sm:justify-start gap-1.5 min-w-0">
                    ...
                    <span className="truncate">{caseData.client_email || "N/A"}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-center sm:justify-start gap-1.5 md:col-span-2 min-w-0">
                    ...
                    <span className="truncate">{caseData.client_address || "N/A"}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-4">
                <DetailRow icon={Scale} label="Case Number" value={caseData.case_number} />
                <DetailRow icon={Building2} label="Court" value={caseData.court_name} />
                <DetailRow icon={Briefcase} label="Category" value={caseData.category_name} />
                <DetailRow icon={Briefcase} label="Sub-Category" value={caseData.sub_category_name} />
              </div>
              <div className="space-y-4">
                <DetailRow
                  icon={Calendar}
            ...
            {/* Opposing Parties */}
            {caseData.opposing_parties && caseData.opposing_parties.length > 0 && (
            ...
            {/* Responsible Lawyers */}
            {caseData.responsible_lawyers && caseData.responsible_lawyers.length > 0 && (
              <div className="border-t border-gray-100 pt-4">
                  ...
                  {caseData.responsible_lawyers.map((lawyer) => (
                    ...
                    >
                      <Avatar className="w-8 h-8 bg-gray-200 shrink-0">
                        <AvatarImage src={getClientImageUrl(lawyer.profile_image || undefined)} alt={lawyer.full_name} />
            ...
            {caseData.note && (
              <div className="border-t border-gray-100 pt-4">
                ...
                <div className="p-3.5 bg-yellow-50/30 border border-yellow-100/50 rounded-2xl flex gap-2">
