...
export default function AdminAllDeadlinesTable({ deadlines = [] }: AdminAllDeadlinesTableProps) {
  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-gray-100 shadow-sm p-4 sm:p-6 md:p-8">
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-[#1e293b]">{t("all_deadlines")}</h2>
        <p className="text-sm text-slate-500 mt-1">{t("view_all_deadlines", "View all deadlines")}</p>
      </div>
            ...
            <thead>
              <tr className="bg-[#eef2f6]">
                <th className="px-6 py-4 border-b border-r border-slate-200 text-left text-sm font-semibold text-slate-600 w-1/3">
                  {formatHeader(t("deadline", "Deadline"))}
                </th>
            ...
            <tbody>
              {deadlines && deadlines.length > 0 ? (
                deadlines.map((deadline, idx) => {
                  ...
                  const { date, time } = formatDeadlineDate(deadline.date);

                  return (
                    <tr key={deadline.id} className="hover:bg-slate-50/30 transition-colors">
                      {/* Date & Time Column */}
                      ...
                      <td className={`px-6 py-4 ${borderBottomClass} text-left`}>
                        <span className={getStatusStyles(deadline.status)}>
                          {displayStatus(deadline.status)}
                        </span>
                      </td>
                    </tr>
                  );
      ...
      <div className="block md:hidden space-y-4">
        {deadlines && deadlines.length > 0 ? (
          deadlines.map((deadline) => {
            const { date, time } = formatDeadlineDate(deadline.date);
            return (
              <div key={deadline.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-3">
